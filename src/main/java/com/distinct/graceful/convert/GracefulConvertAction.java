...
public class GracefulConvertAction extends AnAction {
    ...
    public static boolean isDiffObjAndNotJavaBaseType(PsiType to, PsiType from) {
        return !isJavaBaseType(to.getCanonicalText()) || !to.getCanonicalText().equals(from.getCanonicalText());
    }

    private static boolean isJavaBaseType(String canonicalText) {
        return canonicalText.startsWith("java.") || canonicalText.startsWith("javax.");
    }
    ...
    public static class ObjectGenerateTemplate {
        ...
        private static void generateForField(PsiClass paramClass, StringBuilder builder, PsiField field) {
            ...
            } else if (isDiffObjAndNotJavaBaseType(returnFieldType, paramFieldType)) {
        ...
        private static void generateForObjectField(StringBuilder builder, PsiField field, PsiField paramField, PsiType paramFieldType, PsiType returnFieldType) {
            ...
            if (isDiffObjAndNotJavaBaseType(returnFieldType, paramFieldType)) {
                return;
            }
        ...
        private static void generateForCollectField(StringBuilder builder, PsiField field, PsiType paramFieldType, PsiType returnFieldType) {
            ...
            if (isSameSubType(returnFieldType, paramFieldType)) {
                builder.append(TARGET_NAME + ".set" + getFirstUpperCase(field.getNameIdentifier().getText()) + getFieldNameMethodText(field) + ");\n");
            } else {
                builder.append(TARGET_NAME + ".set" + getFirstUpperCase(field.getNameIdentifier().getText()) + "(convert2" + returnCollectClassType.getPresentableText() + "s" + getFieldNameMethodText(field) + "));\n");
                GenerateTemplateManager.generate(returnFieldType, paramFieldType);
            }
    ...
    private static String getFieldNameMethodText(PsiField field) {
        return "(source.get" + getFirstUpperCase(field.getNameIdentifier().getText()) + "()";
    }
